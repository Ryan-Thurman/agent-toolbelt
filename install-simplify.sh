#!/usr/bin/env bash
# Install the simplify pack into another repo or working folder.
#
# Installs:
#   - .cursor/commands/*.md and .claude/commands/*.md   /simplify and /code-smell
#   - skills/simplify/**                                 shared skill (SKILL.md + references)
#   - .agents/skills/simplify/**                         repo-scoped Codex copy of the skill tree
#
...
usage() {
  cat <<'EOF'
Usage:
  ./install-simplify.sh [options] <target-folder>
...
Examples:
  ./install-simplify.sh ~/work/my-project
  ./install-simplify.sh --dry-run ~/work/my-project
  ./install-simplify.sh --force ~/work/my-project

What gets installed:
  .cursor/commands/       /simplify and /code-smell for Cursor
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --help|-h)
      ...
      ;;
    -*)
      echo "install-simplify: unknown option: $1" >&2
      usage >&2
      exit 2
      ;;
    *)
      if [ -n "$TARGET" ]; then
        echo "install-simplify: only one target folder is allowed" >&2
        exit 2
      fi
      TARGET="$1"
      ;;
...
echo "Installing simplify pack into: $TARGET"
...
for rel in $skill_files; do
  install_tree_file "$rel" "$ROOT/skills/simplify" "$TARGET/skills/simplify"
  install_tree_file "$rel" "$ROOT/skills/simplify" "$TARGET/.agents/skills/simplify"
done
...
echo "Next step: open the target folder in Cursor or Claude Code and run /simplify (or /code-smell)."
echo "In Codex, invoke the skill with /skills or by mentioning \$simplify."
